...
function toProgress(current, base) {
    if (base <= 0)
        return 0;
    return Math.min((current / base) * 100, 100);
}
export function getGroupBuyStatus(input, now = new Date()) {
    ...
    const invalidQuantity = !Number.isFinite(currentQuantity) ||
    ...
    if (invalidQuantity || !deadline) {
        return createResult('invalid_config', Number.isFinite(currentQuantity) ? currentQuantity : 0, Number.isFinite(minQuantity) ? minQuantity : 0, Number.isFinite(targetQuantity) ? targetQuantity : 0, deadline, false);
    }
    const isDeadlineClosed = deadline.getTime() <= now.getTime();
    if (!isDeadlineClosed && currentQuantity >= targetQuantity) {
        return createResult('target_reached', currentQuantity, minQuantity, targetQuantity, deadline, isDeadlineClosed);
    }
    if (!isDeadlineClosed) {
        return createResult('recruiting', currentQuantity, minQuantity, targetQuantity, deadline, isDeadlineClosed);
    }
    if (currentQuantity >= minQuantity) {
        return createResult('deadline_closed', currentQuantity, minQuantity, targetQuantity, deadline, isDeadlineClosed);
    }
    return createResult('failed_minimum', currentQuantity, minQuantity, targetQuantity, deadline, isDeadlineClosed);
}
function createResult(status, currentQuantity, minQuantity, targetQuantity, deadline, isDeadlineClosed) {
    return {
        status,
