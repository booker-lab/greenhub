  ...
  return (
    <Box component="main" style={{ minHeight: '100vh', backgroundColor: 'var(--color-surface-muted)' }}>
      ...
      <Container size="sm" px="md" py="md">
        {error && <Text style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-danger)' }} mb="sm">{error}</Text>}
              ...
              <Paper key={hub.id} radius="lg" px="md" py="md" shadow="xs">
                <Group justify="space-between" align="flex-start" gap="xs">
                  <UnstyledButton component={Link} href={`/hubs/${hub.id}`} style={{ flex: 1, minWidth: 0 }}>
                    <Group gap="xs" mb={4}>
                      <Text style={{ fontWeight: 'var(--fw-medium)', fontSize: 'var(--font-size-sm)' }} truncate>{hub.name}</Text>
                      <Badge
                    ...
                    <Text style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-disabled)' }} truncate>{hub.address}</Text>
                    {hub.addressDetail && (
                      <Text style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-disabled)' }}>{hub.addressDetail}</Text>
                    )}
                    {hub.operatingHours && (
                      <Text style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-disabled)' }} mt={4}>운영: {hub.operatingHours}</Text>
                    )}
                  </UnstyledButton>
