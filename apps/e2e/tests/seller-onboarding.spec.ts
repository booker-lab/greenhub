...
import { ADMIN_STATE_PATH, AUTH_STATE_PATH } from './_helpers/auth';
...
test.describe('셀러 온보딩 — 인증', () => {
  ...
  test('#49 — 일반 셀러 설정에는 관리자 콘솔 이동 링크가 없다', async ({ page }) => {
    await page.goto(`${BASE}/settings`, { waitUntil: 'domcontentloaded' });
    await expect(page.getByRole('heading', { name: '설정' })).toBeVisible({ timeout: 10_000 });
    await expect(page.getByText('사업자 프로필 수정')).toBeVisible();
    await expect(page.getByText('관리자 콘솔로 이동')).toHaveCount(0);
  });

  test('#50 — 일반 셀러 사업자 프로필 수정은 온보딩 자기 프로필로 진입한다', async ({ page }) => {
    await page.goto(`${BASE}/settings`, { waitUntil: 'domcontentloaded' });
    await page.getByText('사업자 프로필 수정').click();
    await expect(page).toHaveURL(/\/onboarding/, { timeout: 10_000 });
    await expect(page.locator('text=사업자 정보 등록')).toBeVisible({ timeout: 10_000 });

    const nameInput = page.locator('input[name="name"]');
    await expect(nameInput).toBeVisible();
    await expect.poll(() => nameInput.inputValue()).not.toBe('');
  });
...
test.describe('셀러 온보딩 — 순수 어드민', () => {
  test.use({ storageState: ADMIN_STATE_PATH });
  test.skip(!adminEmail || !adminPassword, '환경변수 TEST_ADMIN_EMAIL / TEST_ADMIN_PASSWORD 필요');

  test('#51 — storeId 없는 순수 어드민은 온보딩에서 관리자 스토어 목록으로 이동한다', async ({
    page,
  }) => {
