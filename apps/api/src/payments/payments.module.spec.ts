...
import { createPortoneProvider, PaymentsModule } from './payments.module';
...
describe('결제 provider 선택 계약', () => {
  it('일반 실행은 기존 PortoneClient를 유지한다', () => {
    const provider = createPortoneProvider(
      config({
    ...
    expect(provider).toBeInstanceOf(PortoneClient);
    expect(provider).not.toBeInstanceOf(E2EPortoneClient);
  });

  it('검증된 비운영 stub mode에서만 E2EPortoneClient를 선택한다', () => {
    const provider = createPortoneProvider(
      config({
        NODE_ENV: 'test',
    ...
    expect(provider).toBeInstanceOf(E2EPortoneClient);
  });

  it('PaymentsModule은 PortoneClient token을 factory provider로 한 번만 등록한다', () => {
    const providers = (Reflect.getMetadata(MODULE_METADATA.PROVIDERS, PaymentsModule) ?? []) as Array<
      Record<string, unknown>
    >;
    const matches = providers.filter((provider) => provider['provide'] === PortoneClient);
