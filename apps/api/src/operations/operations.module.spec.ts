...
describe('운영 예외 모듈 연결', () => {
  ...
  it('AppModule과 기존 생성 경로 모듈은 OperationsModule을 연결한다', () => {
    ...
    expect(metadata(NotificationsModule, MODULE_METADATA.IMPORTS)).toContainEqual(
      expect.objectContaining({ forwardRef: expect.any(Function) }),
    );
    expect(metadata(OrdersModule, MODULE_METADATA.IMPORTS)).toContain(OperationsModule);
  });
});
