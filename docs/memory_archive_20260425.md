...
| 2 | 디자인 시스템 1~3단계 완료 (방향확정·토큰정비·컴포넌트 var() 리팩토링) | ✅ (components/ 한정) |
| 3 | **디자인 시스템 4단계**: 모바일 UX 개선 (components/ 9개) | ✅ |
| 4 | 디자인 시스템 감사 — app/ 페이지 파일 전체 미수행 확인 | ✅ |
| 5 | 미수행 작업 아토믹 플랜 수립 (T0~T13, 13개 태스크) | ✅ |

## ⚡ 다음 세션 착수 내용

**`docs/specs/design-system-refactor-plan.md` 를 열고 T0부터 순서대로 진행**

| 태스크 | 대상 파일 | 라인 | 상태 |
|--------|---------|------|------|
| T0 | `packages/ui/src/style.css` — 시맨틱 토큰 8개 추가 | - | ⬜ |
| T1 | `components/GreenLoveBrandSection.tsx` — gradient 제거, var() 정규화 | 48 | ⬜ |
| T2 | `app/login/page.tsx` | 106 | ⬜ |
| T3 | `app/order/success/page.tsx` | 116 | ⬜ |
| T4 | `app/search/page.tsx` — SVG→Lucide Search | 102 | ⬜ |
| T5 | `app/cart/page.tsx` | 157 | ⬜ |
| T6 | `app/category/page.tsx` | 171 | ⬜ |
| T7 | `app/checkout/page.tsx` | 379 | ⬜ |
| T8 | `app/products/[id]/page.tsx` — ⚠️ 500라인 경계 | 495 | ⬜ |
| T9 | `app/mypage/_client.tsx` — STATUS_COLORS 전면 교체 | 225 | ⬜ |
| T10 | `app/mypage/notifications/_client.tsx` | 197 | ⬜ |
| T11 | `app/mypage/addresses/_client.tsx` | 212 | ⬜ |
| T12 | `app/mypage/orders/[id]/_client.tsx` | 265 | ⬜ |
| T13 | 타입체크 + e2e 28개 검증 | - | ⬜ |
...
- **아이콘**: Lucide 전용 (커스텀 SVG 금지)
- **라디우스**: 16px 고정
- **스타일**: `packages/ui/src/style.css` CSS var() 단일 출처, 하드코딩 금지
- **모바일**: `--touch-target: 44px`, `--color-danger: #ef4444`, `touch-action: manipulation`
- **세부 지침**: `docs/specs/design-system.md` 참조
- **플랜 전문**: `docs/specs/design-system-refactor-plan.md` 참조
