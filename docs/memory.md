...
- 2026-06-23 배포 게이트 완료. `codex/consumer-mypage-order-confirm` push 완료, Preview `dpl_ECfJ6YwzSD9Rnt4eTQ6BnSnPiNFK` READY 및 branch alias 육안검증 통과, 동일 artifact Production 승격 `dpl_2P2LCSpg2QYGEKagynvcQnKBY1Qq` READY, `greenlove.co.kr` 200 및 `www.greenlove.co.kr` 308→200 확인. 고유 Preview URL은 운영 API CORS로 데이터 fetch가 막혀 branch alias 기준으로 검증했다.
...
- 2026-06-23 소비자앱 추가작업 잔여 확인 완료. 발견 큐, 종료 체크리스트, 주소 보정 계획을 실제 상태에 맞춰 갱신했고 사용자 흐름 기준 남은 구현 작업은 없다고 판정했다. 당시 남은 비기능 항목은 CSS preload warning 추적과 검증 캡처 보강 여부 점검이었다.
...
- 2026-06-24 검증 캡처 보강 여부 점검 완료. 이미지 캡처 파일은 별도 발견되지 않았지만 W0~W11의 Preview·Production 배포 증거, 375px·1440px 육안검증 관찰 기록, CSS warning 6개 URL 재확인 결과가 있어 추가 캡처를 필수 잔여 작업으로 보지 않는다. 새 결함, 배포 회귀, 외부 감사 요청이 있을 때만 별도 계획으로 캡처를 추가한다.
- 2026-06-24 소비자앱 추가 작업 육안 검증 리스트 작성. 로컬 정상 조합은 API `localhost:3000`, consumer `localhost:3001`이며 3002는 CORS로 데이터 검증에 부적합함을 확인했다. 홈, 카테고리 확장 필터, 가격 필터 URL 반영, 링크 복사, 상품 상세/카테고리 복귀, 상점 목록/상세를 375px·1440px에서 Playwright+Chrome으로 점검했고 스크린샷은 `test-results/manual-consumer-visual-20260624-final`에 남겼다. 비인증 상태의 NextAuth session fetch 경고는 관찰됐으나 공개 탐색 플로우는 계속 동작했다.
- 2026-06-24 운영 도메인 `greenlove.co.kr` 직접 육안 검증. 홈, 카테고리 노출, 링크 복사, 상품 상세/카테고리 복귀, 상점 목록/상세는 375px·1440px에서 통과했고 스크린샷은 `test-results/domain-consumer-visual-20260624`에 남겼다. 단, 가격 필터는 입력값 `1000`이 필드에 들어가도 `적용` 후 `priceMin=1000`이 URL에 반영되지 않아 운영 결함 후보로 분리했다.
- 2026-06-29 docs 문서 정리 완료. `docs/README.md`, `docs/specs/README.md`, `docs/specs/frontend/README.md`, `docs/archive/README.md`를 진입점으로 추가했고, `seller-refactor-visual-verify.md`의 장문 F-VISUAL-PATH/M-PATH를 별도 문서로 분리했다. 500라인 초과 archive 원문 3개는 기존 경로를 인덱스로 유지하고 part 문서로 분할했다. 결정 로그는 #CL-162에 기록했다.
- 2026-07-01 k6 부하테스트 계획과 실행 골격을 추가하고 production 읽기 전용 smoke를 실행했다. `docs/specs/ops/k6-load-test-plan.md`에 smoke, baseline, launch, growth, spike, soak 순서의 목표치 증분 계획을 고정했고, `tests/load/`에 공개 조회, checkout, seller, admin, driver, throttle, readiness 시나리오를 추가했다. `GrafanaLabs.k6` v2.0.0을 winget으로 설치했으며 `smoke.js`, 실제 공개 ID 기반 `consumer-read.js`, 읽기 전용 `checkout.js`, 최소 반복 `release-readiness.js` smoke와 재검증이 통과했다. 결과는 `docs/performance/load/`에 저장했다. production baseline 이상은 전역 `100 req/min` 제한 때문에 staging 또는 preview에서 별도 정책을 둔 뒤 실행한다.
- 2026-07-01 카카오 로그인 보안 보강과 k6 분리 계획을 문서화했다. `docs/plans/PLAN_kakao-auth-hardening-loadtest-handoff.md`에 A0~A7 원자 작업, 정합성 검토, 새 대화 핸드오프 프롬프트를 작성했다. `docs/specs/api/auth.md`는 `/auth/kakao-login`이 `kakaoAccessToken`으로 API 서버 측 사용자 검증을 수행하는 계약으로 갱신했고, `docs/specs/ops/k6-load-test-plan.md`에는 카카오 OAuth 반복 호출을 부하테스트에서 제외한다고 명시했다. 결정 로그는 #CL-164에 기록했다.
- 2026-07-01 카카오 로그인 보안 보강 구현 완료. API `KakaoLoginDto`는 `kakaoAccessToken` 중심 계약으로 바뀌었고 `KakaoClient`가 카카오 사용자 정보를 직접 조회한 검증 프로필만 `AuthService.kakaoLogin`에 공급한다. consumer/seller/driver NextAuth Kakao callback은 `account.access_token`만 API로 전달한다. 회귀 테스트는 `kakao.client.spec.ts`, `kakao-login.dto.spec.ts`, `auth.service.spec.ts`에 추가했고 `corepack pnpm --filter api test`, `corepack pnpm --filter api build`, consumer/seller/driver `tsc --noEmit`이 통과했다. 전체 API lint는 기존 광범위한 `any` 오류로 실패했고, `node scripts/load/run-k6`는 기본 `localhost:3000` API 미기동으로 연결 거부 실패했으며 결과 JSON은 `docs/performance/load/readiness-smoke-2026-06-30T15-51-45-551Z.json`에 생성됐다.
- 2026-07-01 카카오 로그인 보안 보강 후속 검증 완료. 로컬 API `localhost:3000`에서 `node scripts/load/run-k6` readiness smoke를 실제 읽기용 `K6_STORE_ID`/`K6_PRODUCT_ID`, `K6_ENABLE_WRITES=false`, credential 미설정으로 재실행해 통과했고 결과는 `docs/performance/load/readiness-smoke-2026-06-30T15-59-34-639Z.json`에 저장했다. placeholder ID 기반 첫 재시도는 404가 `http_req_failed` threshold에 잡혀 실패했으나 쓰기 요청은 없었다. `docs/archive/CRITICAL_LOGIC_archive_20260516_part-03.md`가 553라인이라 `part-04`로 분할해 모든 변경 파일을 500라인 이하로 맞췄다.
- 2026-07-01 카카오 로그인 보안 보강 마감 검증을 재수행했다. `normalizeHubIds`를 `hub-ids.util.ts`로 분리해 `auth.service.ts`를 500라인 이하로 낮췄고, `corepack pnpm --filter api build`, `corepack pnpm --filter api test` 21 suites/140 tests, consumer/seller/driver `tsc --noEmit`, consumer/seller/driver lint, `git diff --check`를 통과했다. API 전체 ESLint의 기존 `any`/unsafe 부채는 `docs/BACKLOG.md`의 `API-LINT-BASELINE`으로 분리했다.
