...
최종 수정: 2026-07-18 (원 계획 Task 5.7 셀러 설정 운영 메뉴 완료)
## 현재 진행

- 브랜치: `codex/mvp-sales-round-direct`
- Task 5.7 시작 SHA: `caafbbdb9565d031999e092a26827f82e8fed7f6`
...
- 완료: Task 5.7 `apps/seller/src/app/settings/page.tsx`
- 다음: Task 5.8 `apps/seller/src/app/orders/page.tsx`
...
- 설정 화면에 정산 관리 `/settlements` 접근을 추가했다.
- 기존 배송 설정 `/settings/delivery`, 배송 슬롯 `/settings/daily-caps`, 거점 관리 `/hubs` 접근을 유지했다.
- 네 운영 경로를 `SELLER_OPERATION_SETTINGS`로 고정하고 인접 Vitest 1개로 검증했다.
- 계정·관리자·정보 섹션과 Task 5.6 주문·회차·상품·준비·설정 하단 메뉴는 변경하지 않았다.
- 주문·드라이버 화면과 `test.fixme` 계약은 변경하지 않았다.
...
- Task 5.8 주문 화면은 시작하지 않았다.

## 후속 Task 실행 원칙

- 다음 작업은 Task 5.8이며 주문 화면에서 배송 보류와 확인 필요 건수만 업무 우선순위로 표시한다.
