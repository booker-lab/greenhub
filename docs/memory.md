...
최종 수정: 2026-05-28 (#CL-55 users S1 — refresh 정지 차단 구현·branch push, S2 인계 준비)
...
- **users S1 (2026-05-27~28, #CL-55, branch push)**: 정지된 소비자가 refresh token으로 access를 최대 30일 재발급할 수 있던 결함을 차단. `auth.service.ts` `refresh()`에서 rotation 검증 후 `users/{sub}.suspended`를 검사해 true이면 기존 `auth.login.suspended` 감사 이벤트와 `401`을 반환하고, access 자연 만료 최대 1시간 정책은 유지했다. `auth.service.spec.ts` Jest 2케이스(정지 차단·정상 rotation) 신설. 검증은 API Jest 4/4·`tsc --noEmit`·build 통과; API 전역 ESLint는 기존 `408 errors / 20 warnings` baseline으로 실패했으며 신규 spec/추가 블록의 신규 Biome 진단은 0. 커밋 `be5def9`를 `codex/admin-users-s1-refresh-suspend`에 push했고 `main` 병합·배포·S5 live E2E는 대기 중. **다음 = users S2(T1+T2: 가입일·전화 표시 및 reload 버튼).**
