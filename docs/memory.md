...
최종 수정: 2026-07-15 (회차 직배송 Task 2.9 결제 복구 테스트 계약 완료)
...
- Phase 2.9 완료: 기존 결제 보정 테스트를 보존하고 타임아웃 결제 재조회, 늦은 결제 한도 재확보, 재확보 실패 자동 전액 환불 계약을 `payments.service.spec.ts`에 고정.
...
- `pnpm --filter api test -- --listTests` 통과: `payments.service.spec.ts`를 포함한 8개 테스트 파일 수집.
- `pnpm --filter api test -- payments.service.spec.ts --runInBand` 예상 실패: 기존 계약 2개 통과, Task 2.10 구현 대기 계약 4개 실패.
- `pnpm --filter api build`, `git diff --check` 통과.
...
- `docs/plans/PLAN_mvp_sales_round_direct_delivery.md`의 Task 2.10이 다음 실행 진입점이다.
- Task 2.10은 `apps/api/src/payments/payments.service.ts` 단일 Target에서 타임아웃 포트원 재조회와 주문·재배송비 결제 멱등 확정을 구현한다.
- PortOne 샌드박스 검증은 원 계획 Task 2.10 전, Firebase Emulator 검증은 원 계획 Task 6.4 전에 완료한다.
