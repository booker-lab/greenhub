...
- 2026-06-17 소비자 MY 주문 안심 개선 구현. MY 목록 표시 모델과 주문 상세 행동 모델을 `_lib.ts`로 분리하고, 공동구매 구분·행동 신호·수령/운송장 안내·구매 확정 오류 표시·refetch 결함을 정리했다. `pnpm --filter consumer lint`, `pnpm --filter consumer build` 통과. `consumer-mypage.spec.ts`는 기본 원격 `greenlove.co.kr`의 고정 주문 ID 조회 실패로 마지막 택배 상세 케이스만 실패하고 나머지 10개는 통과했다.
