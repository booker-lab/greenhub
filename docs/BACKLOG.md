...
| 🟡 P2 | **[PREVIEW-GATE] sync-preview 배포 게이트(B 해소)** — CI-SEED §범위 외로 분리된 stale preview race. sync-preview success ≠ Vercel 실배포 완료라 자동 e2e가 stale 배포 검사(세션39·60·61). 메모리 권장 "수동 5분 대기"는 사람 수동 루프. 해법: e2e dispatch 전 3앱 Preview deployment의 `sha==preview HEAD`+status `success` **폴링**(timeout fail-fast) + `permissions:deployments:read`. **세션71 진단·플랜 완료**(아토믹 T0~T5, 정합성 8항목 실측, 사용자 결정 2건: 폴링 방식·fail-fast). 구현은 차기 세션. #CL-43 예정 | 인프라/DX | `specs/api/preview-deploy-gate-plan.md` (세션71 수립) |
