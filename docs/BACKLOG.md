...
| UX-07 | Medium | 셀러앱 탭 스타일 2종 혼재 — 주문(검정 underline) vs 상품·정산(초록) | 📋 세션53 플랜 T-UX1 |
| UX-08 | Medium | 셀러 상품 카드 Badge-as-button — 상태 표시와 액션 버튼 시각 미구분 | 📋 세션53 플랜 T-UX2 |
| UX-09 | Low | 셀러앱 `confirm()` vs Modal 혼재 — 삭제 확인 패턴 불일치 (6건 잔존) | 📋 세션53 플랜 T-UX3 |
...
| 🟢 P3 | **셀러 UX 잔여(UX-07~09) 정합** — T-UX1 탭 단일화·T-UX2 상품 카드 Badge 분리·T-UX3 ConfirmModal 6곳 교체·T-UX4a/b/c fontSize 토큰화·T-UX5 정합성 검토. UX-10은 자연 해소(⏹️). Railway 무관 | UX/DX | `specs/frontend/seller-ux-residual-plan.md` (세션53 수립) |
