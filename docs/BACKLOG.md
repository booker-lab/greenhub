...
| 🟢 P3 | **셀러앱 정리 작업(T-CLEAN1~3)** — 세션 28~60 리팩토링 종합 점검 도출 후속 3건. ✅ T-CLEAN1 biome baseline 정리(40e/16w → **0e/2w**, 세션62 완료 `2f100e1`+`09061df`)·✅ T-CLEAN2 native `alert()` 3건 → `@mantine/notifications`(0건 달성·#CL-39 등재, 세션63 완료 `80a7e51`+`35f8410`)·T-CLEAN3 products/page.tsx → `apiJson` 마이그레이션(#CL-32 P2 잔여분, 세션64). 각 세션 진입 시 사전 정합성 검토 후 진입. Railway 무관 | DX/UX | `specs/frontend/seller-cleanup-plan.md` (세션61 수립) |
