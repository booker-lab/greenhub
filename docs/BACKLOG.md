...
| 🟢 P3 | **[T-CLEAN3-B] 잔존 `apiFetch` 8파일 → `apiJson` 점진 마이그레이션** — 세션64에서 ProductCard만 Phase A로 봉합 후 별건 분리. 대상 8파일: `hubs/page.tsx`·`hubs/new/page.tsx`·`hubs/[id]/page.tsx`·`hubs/[id]/pickup/page.tsx`·`settlements/_hooks/useSettlements.ts`·`settings/daily-caps/page.tsx`·`settings/delivery/page.tsx`·`onboarding/page.tsx`. 멀티파트/스트리밍은 raw `apiFetch` 유지(인프라 함수). 에러 메시지 톤은 useAdmin 계열(`ApiError.message` 우선 + 사용자 친화 폴백)로 통일. Railway 무관 | DX | 세션64 도출 (T-CLEAN3 plan §Phase B) |
