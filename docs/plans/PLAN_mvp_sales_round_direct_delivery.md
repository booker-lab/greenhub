...
## Closeout Roll-up
- **Status**: Task 4.3 완료, Task 4.4 `todo`
...
- **검증 결과**: Task 4.3 Node 테스트 6개, consumer 타입검사, 전체 typecheck와 build, Playwright chromium·mobile 24개 목록 수집, 변경 파일 Biome 오류 수준 검사와 `git diff --check`가 통과했다.
- **다음 진입점**: Task 4.4 `apps/consumer/src/components/HomeProductList.tsx`만 시작할 수 있다. 현재 회차·마감·직배송 조건·지난 회차 순서의 홈 구성과 유입 캡처 연결만 구현하고 Task 4.5 이후 목록·내비게이션·상세·장바구니·결제 화면을 선행하지 않는다.
