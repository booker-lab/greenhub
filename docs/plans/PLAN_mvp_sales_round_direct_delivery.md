...
## 문서 메타
- **작성일**: 2026-07-15
- **상태**: Task 4.10 완료, Task 4.11 착수 대기
- **Priority**: 1
...
## Closeout Roll-up
- **Status**: Task 4.10 완료, Task 4.11 `todo`
...
- **검증 결과**: Task 4.10 전용 Node 테스트 5개, Task 4.8 상세 계약 5개, Task 4.9 패널 계약 6개 등 Node 테스트 16개, consumer 타입검사, 전체 typecheck와 build, Playwright chromium·mobile 24개 목록 수집, 변경 파일 Biome 오류 수준 검사와 `git diff --check`가 통과했다. 전체 build가 갱신한 tracked 생성물 5개는 시작 HEAD 상태로 복원해 범위에서 제외했다.
- **다음 진입점**: Task 4.11 `apps/consumer/src/hooks/useCart.ts`만 시작할 수 있다. Task 4.10의 회차 구매 동작에 검증된 회차·회차 상품·회차 가격을 저장하고 같은 회차 상품만 묶되 결제 화면·주문 요청 연결은 Task 4.13~4.15보다 앞서 구현하지 않는다.
