...
## 문서 메타
- **작성일**: 2026-07-15
- **상태**: Task 3.10 완료, Task 3.11 착수 대기
- **Priority**: 1
...
## Closeout Roll-up
- **Status**: Task 3.10 완료, Task 3.11 `todo`
- **검증 결과**: `RetentionService.saveRecord`가 목적별 분리 컬렉션과 90일·3년·5년 `expiresAt`을 Firestore Timestamp로 저장한다. `purgeExpiredRecords`는 네 컬렉션을 각각 `expiresAt <= now`로 조회하고 `OPEN` 분쟁과 `legalHold`를 제외하며, 만료된 연결 사진은 주입된 Storage 어댑터로 삭제하고 문서는 Firestore 단일 배치로 삭제한다. 빈 대상과 성공 후 재실행에는 추가 부작용이 없다. 신규 8개와 기존 6개 스위트 55개 테스트, API 빌드, Biome 오류 수준 검사와 `git diff --check`가 통과했다.
- **잔여 위험**: `RetentionService`는 아직 모듈과 런타임 경로에 연결되지 않았고 실제 Storage 구현도 없다. 다음 Task 3.11에서는 `retention.module.ts`만 추가해 서비스를 내보내며 주문·결제·알림의 실제 호출 연결은 계획된 후속 범위를 지켜야 한다.
