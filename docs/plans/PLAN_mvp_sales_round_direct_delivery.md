...
## Closeout Roll-up
- **Status**: Task 4.8 완료, Task 4.9 `todo`
...
- **검증 결과**: Task 4.8 Node 테스트 5개, consumer 타입검사, 전체 typecheck와 build, Playwright chromium·mobile 24개 목록 수집, 변경 파일 Biome 오류 수준 검사와 `git diff --check`가 통과했다. 전체 build가 갱신한 tracked 생성물 5개는 시작 HEAD 상태로 복원해 범위에서 제외했다.
- **다음 진입점**: Task 4.9 `apps/consumer/src/app/products/[id]/_components/RoundPurchasePanel.tsx`만 시작할 수 있다. Task 4.8이 전달한 회차 상품 상태를 사용해 회차 가격·마감·배송 고지와 구매 가능 상태를 표시하고 Task 4.10 이후 ProductActions 단순화·장바구니·결제 화면을 선행하지 않는다.
