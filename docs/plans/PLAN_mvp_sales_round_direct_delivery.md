...
## 문서 메타
- **작성일**: 2026-07-15
- **상태**: Task 3.13 완료, Task 3.14 착수 대기
- **Priority**: 1
...
## Closeout Roll-up
- **Status**: Task 3.13 완료, Task 3.14 `todo`
- **검증 결과**: 서버 Storage 서비스가 Firebase 인프라 모듈의 provider/export로 연결되고, 기존 `FIREBASE_APP` 초기화에 명시적 또는 project ID 기반 기본 bucket 계약을 적용했으며, 보관 서비스의 삭제 어댑터가 `StorageService` 토큰으로 주입된다. 신규 모듈 계약 3개를 포함한 보관·Storage·운영 예외·알림·결제·주문 관련 15개 스위트 98개 테스트, API 빌드, 변경 TypeScript 파일 Biome 오류 수준 검사와 `git diff --check`가 통과했다.
- **잔여 위험**: `AppModule`은 아직 `RetentionModule`을 최종 연결하지 않아 보관 서비스의 애플리케이션 런타임 진입점이 없으며, 주문·드라이버의 배송 사진 실제 호출 경로도 아직 연결되지 않았다. 다음 Task 3.14에서는 회차·운영 예외·보관 모듈을 애플리케이션에 최종 등록하되 공개 URL·클라이언트 직접 업로드·공개 Storage 권한을 추가하지 않고, 실제 주문·드라이버 호출 경로는 계획된 후속 Task 범위보다 선행하지 않아야 한다.
