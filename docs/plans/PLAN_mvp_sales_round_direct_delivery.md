...
## 문서 메타
- **작성일**: 2026-07-15
- **상태**: Task 4.9 완료, Task 4.10 착수 대기
- **Priority**: 1
...
## Closeout Roll-up
- **Status**: Task 4.9 완료, Task 4.10 `todo`
...
- **검증 결과**: Task 4.9 전용 Node 테스트 6개와 Task 4.8 상세 계약을 포함한 Node 테스트 11개, consumer 타입검사, 전체 typecheck와 build, Playwright chromium·mobile 24개 목록 수집, 변경 파일 Biome 오류 수준 검사와 `git diff --check`가 통과했다. 전체 build가 갱신한 tracked 생성물 5개는 시작 HEAD 상태로 복원해 범위에서 제외했다.
- **다음 진입점**: Task 4.10 `apps/consumer/src/app/products/[id]/_components/ProductActions.tsx`만 시작할 수 있다. 회차 패널의 구매 가능 상태를 구매 동작에 위임하고 택배·거점·공동구매·배송일 선택을 제거하되 Task 4.11 이후 장바구니 모델·결제 화면을 선행하지 않는다.
