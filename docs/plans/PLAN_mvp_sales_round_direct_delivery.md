...
## 문서 메타
- **작성일**: 2026-07-15
- **상태**: Task 4.12 완료, Task 4.13 착수 대기
- **Priority**: 1
...
## Closeout Roll-up
- **Status**: Task 4.12 완료, Task 4.13 `todo`
...
- **검증 결과**: Task 4.12 전용 Node 테스트 8개, Task 4.8 상세 5개, Task 4.9 패널 6개, Task 4.10 구매 동작 5개, Task 4.11 장바구니 9개 등 Node 테스트 33개, consumer 타입검사, 전체 typecheck와 build, Playwright chromium·mobile 24개 목록 수집, 변경 파일 Biome 오류 수준 검사와 `git diff --check`가 통과했다. 전체 build가 갱신한 tracked 생성물 5개는 시작 HEAD 상태로 복원해 범위에서 제외했다.
- **다음 진입점**: Task 4.13 `apps/consumer/src/hooks/usePayment.ts`만 시작할 수 있다. Task 4.12가 선별한 같은 회차 항목을 한 주문·한 결제로 처리하도록 결제 훅을 확장하되 Task 4.14 이후 결제 화면·주문 요청 변경은 앞서 구현하지 않는다.
