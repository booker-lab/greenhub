...
- **현재 진입점**: 서버 전용 Firestore 컬렉션의 직접 접근 차단과 공개 회차 읽기 계약을 구현했으며 다음 작업은 Task 6.3 Storage 보안 규칙이다. 단, Firestore 에뮬레이터 동적 검증은 JDK 21 이상에서 재실행해야 한다.
...
| 6.2 | 6.1 | `firestore.rules` | 회차 쓰기·예약·법정 기록·운영 예외의 클라이언트 직접 접근을 차단한다. | `pnpm --filter api build` | 구현 완료 — API와 클라이언트 호출부 감사 결과 `checkoutReservations`, `operationIssues`, 법정·마케팅·배송 사진 보관 기록 4종, `notificationDeliveries`는 Admin SDK 서버 경로만 사용하므로 직접 읽기·쓰기를 명시적으로 차단했다. `saleRounds`와 `saleRoundItems`는 공개 상태 회차에 한정한 단건·목록 읽기를 허용하고 작성 중 회차 조회와 모든 클라이언트의 생성·수정·삭제를 차단했다. 관련 API 90개, 전체 타입 검사와 build, Biome 및 공백 검사는 통과했다. 실제 규칙 파일을 로드하는 계약 테스트 13개를 추가했지만 Firebase CLI 15.18.0이 현재 JDK 17에서 JDK 21 이상을 요구해 에뮬레이터는 테스트 본문 전에 중단됐으며 동적 통과로 판정하지 않았다. | done |
...
- **Task 6.2 결과**: 서버 Admin SDK만 사용하는 예약·운영 예외·법정 및 보관 기록·알림 전달 컬렉션 7개의 클라이언트 직접 접근을 명시적으로 차단하고, 공개 상태 회차와 그 회차 상품만 읽도록 허용했다. 인증 없음·일반 사용자·판매자·기사·관리자의 서버 전용 읽기 및 생성·수정·삭제 거부, 공개·비공개 회차 읽기 경계와 쓰기 거부, 기존 공개 데이터와 역할별 주문 읽기를 실제 규칙 파일로 검증하는 계약 테스트 13개를 추가했다. 관련 API 90개, 타입 검사, 전체 build와 정적 검사는 통과했지만 현재 JDK 17에서는 Firebase CLI 15.18.0이 JDK 21 이상을 요구해 에뮬레이터가 테스트 본문 전에 중단됐으므로 동적 통과는 미확인이다.
- **다음 진입점**: Task 6.3 `storage.rules`만 시작할 수 있다. Firestore 및 Storage 에뮬레이터 동적 계약은 JDK 21 이상에서 재실행해야 한다.
- **명시적 후속 위험**: 드라이버 화면 계약 14개, 소비자 화면 계약 24개와 셀러 화면 계약 12개는 실행 데이터와 화면 준비 전이라 계속 `test.fixme`다. 기존 거점 사진은 legacy 경로를 유지하므로 Task 5.12의 회차 비공개 사진 계약으로 승격하지 않았다. Storage 보안 규칙은 Task 6.3, 서버 전체 흐름과 사용자 화면 실행 검증은 Task 6.4·6.7에 남아 있다. Task 6.2 Firestore 계약과 Task 6.3 Storage 계약의 에뮬레이터 검증에는 JDK 21 이상이 선행돼야 하며 `salesMode` 전환과 운영 배포는 Task 6.5 이후 별도 승인 전에는 수행하지 않는다.
