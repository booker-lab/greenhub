...
- **현재 결과**: consumer 개인정보처리방침·이용약관·footer 링크를 main과 production에 반영하고 운영 브라우저 검증을 완료한 뒤 카카오 비즈니스 재심사를 접수함. 심사 결과 대기
...
- 공개 승인된 사업장 주소 외 비공개 전달 Gmail 주소·계정 로그인 이메일·등록증 원본 이미지·생년월일을 기록하지 않음

## 후속 상품 상세 사업자 정보 정합성과 4차 재심사

### Task 4.7 — 상품 상세 판매자 정보 정합성 보강

- **Status**: production 반영 및 운영 검증 완료
- PR #20에서 상품 상세 판매자 주소와 전화번호를 스토어 문서 값 대신 공개 사업자 정본의 주소·고객센터로 통일했다. 상품·스토어 운영 DB 문서는 변경하지 않았다.
- 후속 사업자 푸터 PR #22에서 공식 주소, 호스팅서비스 제공자, 이메일, 고객센터와 상담시간을 공개하고 운영 관계 문장은 상단에만 유지했다. 마감 문서는 PR #21·#23으로 정리했다.
- 상품 상세 상호 아래에 스토어 데이터의 `store.ceoName`이 `난플렉스`로 표시되던 보조 줄을 제거하고 다시 렌더링되지 않도록 계약을 추가했다.
- consumer 계약 테스트 25/25, lint 오류 0건·기존 경고 23건, Next.js 16.2.5 production build와 `git diff --check`가 통과했다.
- 기능 commit `bc51e4dd729b6c5d731a6f79baab0ea91418422a`을 push하고 PR #24의 모든 검사가 통과한 뒤 merge commit `c96db6f3dd0ed5c7f6bd97fca9ab5763b9aed74c`로 main에 병합했다.
- consumer production Vercel 배포 `dpl_4TPxLkDExMDqxBUVRg8nSV6R86kv`와 GitHub deployment `5959074365`가 성공했고 운영 도메인 별칭과 HTTP 200 응답을 확인했다. seller·driver는 Ignored Build Step으로 취소됐다.
- 운영 데스크톱과 375×812 모바일 상품 상세에서 판매자 정보·상호·공개 주소·고객센터는 유지되고 `난플렉스`는 0건이었다. 가로 넘침과 브라우저 콘솔 오류도 0건이었고 consumer Vercel 초기 런타임 오류 로그도 0건이었다.

### Task 4.8 — 4차 비즈니스 재심사 접수

- **Status**: done — 2026년 8월 19일 반려 결과 확인
- 2026년 8월 18일 사용자가 열어 둔 대상 채널 `그린러브`의 카카오 비즈니스 재심사 화면에서 접수를 진행했다.
- 매장 없음, 공개 사업자등록번호, 대표자 전자증명 완료, 인허가 비대상을 선택했다. 사업자등록증 업체명 `디어 오키드`와 채널 이름 `그린러브`가 서로 다른 점은 연관성 설명으로 보완했다.
- 연관성 설명은 카카오톡 채널과 화훼 쇼핑몰을 사업자 `디어 오키드`가 직접 운영하고, 홈페이지 첫 화면의 운영 관계·공식 채널 링크·대표상품과 하단 푸터·상품 상세 판매자 정보에서 동일 사업자 정보를 확인할 수 있다는 내용으로 글자 수 제한 안에서 제출했다.
- 홈페이지 주소는 오기 없이 `https://greenlove.co.kr`로 입력했다. 파일 첨부는 0건이며 사업자등록증 원본, 비공개 계정 정보와 그 밖의 민감정보는 추가 제출하지 않았다.
- 최종 권리·정보 정확성 확인에 동의한 뒤 성공 화면에서 요청 내용 `비즈니스 재심사 신청`, 요청일 `2026. 8. 18.`, 영업일 기준 평균 3~7일 안내를 확인했다.
- 이번 외부 변경은 카카오 재심사 접수 1건뿐이다. 카카오 채널 전화번호·소개·소식, 이메일·DNS·ALIGO, Firebase 플랜·Billing·Storage와 운영 DB는 변경하지 않았다.
- **Verify**: 카카오 재심사 성공 화면의 요청 내용·요청일·예상 심사 기간 확인

### Task 4.9 — 4차 반려 원인과 회차 직배송 원계획의 법적 고지 누락 확인

- **Status**: done — 별도 consumer 법적 고지 작업 필요
- 2026년 8월 19일 카카오 고객센터 답변에서 대상 채널 `그린러브`와 홈페이지 `https://greenlove.co.kr/`의 개인정보처리방침과 이용약관을 확인할 수 없어 사업자채널 연관성 검증이 불가능하다는 반려 사유를 확인했다.
- 운영 홈페이지 응답은 HTTP 200이지만 `개인정보처리방침`, `개인정보 처리방침`, `이용약관` 문구와 관련 링크가 없고 `/privacy`, `/terms`를 포함한 통상적인 공개 경로는 404였다.
- 현재 `BusinessInfoFooter`는 사업자 공개 정보만 렌더링하며 개인정보처리방침과 이용약관 링크나 본문 페이지를 제공하지 않는다.
- 회차 직배송 branch `codex/mvp-sales-round-direct`의 원계획 `PLAN_mvp_sales_round_direct_delivery.md`, 출시 차단 계획 `PLAN_mvp_round_direct_launch_blockers.md`, 중단 인계, 출시 보고서, 사양과 운영 절차를 검색한 결과 개인정보처리방침·이용약관·법적 고지 푸터 작업은 포함돼 있지 않았다.
- 따라서 개인정보처리방침과 이용약관의 실제 서비스 기준 작성, 공개 페이지, 홈페이지 하단 링크, 운영 검증과 카카오 재심사는 이번 별도 consumer 작업에서 반드시 수행한다. 회차 직배송 출시는 이 작업이 main에 병합되고 출시 branch에 통합되기 전에는 재개하지 않는다.
- 실행이 끝난 회차 원계획의 Task 구조는 변경하지 않는다. 회차 branch를 재개할 때 최신 main 통합과 중단 인계의 선행 조건으로 법적 고지 완료 상태를 기록한다.
- 회차 직배송 기본 worktree `C:\Develop\greenhub`에는 예상하지 못한 미추적 `docs/plans/PLAN_kakao_business_channel_homepage_reapproval.md`가 있어 이번 확인에서는 어떤 파일도 수정하지 않았다.
- **Verify**: 원계획 관련 문서의 `개인정보처리방침|개인정보 처리방침|이용약관|푸터|footer` 검색 결과 0건, 운영 공개 경로와 consumer 코드 읽기 전용 대조

## 후속 consumer 법적 고지 구현

### 법적 고지 Task 1.1~1.2 — 실제 처리 흐름과 카카오 증거 정본

- **Status**: done
- consumer·API 코드와 production 인프라를 대조해 카카오 로그인, 회원·주소록, 장바구니·체크아웃 브라우저 저장소, 주문·결제 준비 경로, 알림과 감사 로그의 실제 처리 항목을 정리했다.
- Firestore 기본 저장 위치는 서울 `asia-northeast3`, Railway API는 미국 캘리포니아 `us-west2`이고 consumer 프런트엔드는 Vercel에서 제공되는 사실을 확인했다.
- 현재 상용 PG·주문·배송·마케팅 수신 기능은 운영하지 않는다는 사용자 확인을 문안 경계로 고정했다. 코드·환경변수 이름의 존재를 상용 계약이나 운영 증거로 해석하지 않았다.
- 전자상거래법상 표시·광고 6개월, 계약·청약철회 5년, 대금결제·공급 5년, 불만·분쟁 3년 보존기간을 반영했다. 회원 탈퇴·카카오 연결 해제·일반 데이터 자동 만료 삭제 미구현은 공개 권리 행사 절차와 후속 API 위험으로 함께 기록했다.
- 현재 Vercel 계정은 Hobby이며 공개 Vercel DPA의 Pro·Enterprise 적용 범위가 아닌 점, Railway 미국 API 국외 처리와 판매 개시 전 결제·배송·반품 조건 확정 의무를 내부 법적 정본의 잔여 위험으로 남겼다.
- 공개 경로 `/privacy`·`/terms`의 비로그인 200, 운영자·시행일, footer·상호 링크, 모바일 가시성과 console 오류 0건을 카카오 증거 정본에 추가했다. production 검증 완료와 카카오 재심사 제출 승인은 분리했다.
- **Verify**: 두 SSOT 문서 `git diff --check` 종료 코드 0

### 법적 고지 Task 2.1~2.7 — 계약과 공개 페이지 구현

- **Status**: done
- 공통 `LegalDocumentPage` Server Component와 정적 `/privacy`, `/terms` 경로를 구현했다. 홈·상호 문서 링크, 운영자·시행일, 의미 있는 문서 구조, 44px 터치 영역과 고정 하단 navigation을 피하는 safe-area 포함 여백을 제공한다.
- 개인정보처리방침은 목적·항목·수집 방법, 보유·파기, 제3자 제공, Vercel·Railway·Google 처리위탁과 국외 처리, Kakao 로그인·다음 우편번호 외부 서비스, 필수 쿠키·브라우저 저장소, 이용자 권리·안전조치·구제·변경 절차를 고지한다.
- 이용약관은 현재 공개 정보·로그인·회원·장바구니·주문 준비 범위와 상용 주문·결제·배송 미운영을 분리한다. 판매 개시 전에 결제·직배송·택배·반품 비용·청약철회 조건을 확정하도록 하고 미확정 비용이나 법정 권리를 일괄 배제하는 문구는 넣지 않았다.
- `BusinessInfoFooter`의 사업자 상세 다음에 `개인정보처리방침`, `이용약관` 공개 링크를 추가했다.
- Playwright 검증기는 홈 footer에서 두 문서로 이동하고 다시 상호 이동하면서 HTTP 200, 비로그인 경로 유지, 제목·운영자·시행일, 데스크톱·375×812 가로 넘침, 고정 UI 가림과 브라우저 오류를 확인한다.
- 구현 전 계약은 법적 파일 미존재 3건과 footer 링크 미구현 1건이 의도한 RED였고, 구현 후 전체 계약은 GREEN으로 전환됐다.

### 법적 고지 Task 3.1~3.5 — 배포 전 검증

- **Status**: done — 배포 대기
- 법적 셸·개인정보처리방침·이용약관·홈·footer 계약은 12/12 통과했다.
- `pnpm --filter consumer exec tsc --noEmit`은 타입 오류 0건, 종료 코드 0이었다.
- `pnpm --filter consumer lint`는 오류 0건, 종료 코드 0이었다. 이번 변경 외 checkout·auth·상품 UI 등에 기존 경고 23건이 있으며 새 법적 문서 관련 경고는 없다.
- Next.js 16.2.5 production build는 compile·TypeScript·페이지 데이터 수집과 정적 페이지 15/15 생성을 완료했다. `/privacy`와 `/terms`는 정적 prerender 경로다.
- 최초 로컬 production 브라우저 실행은 환경변수 미주입으로 Auth.js 500과 로컬 API CORS 오류를 검출했다. 비밀값을 저장소에 복사하지 않고 기존 consumer production 로컬 환경을 검증 프로세스에만 주입해 재빌드·재실행했다.
- 재실행 결과 데스크톱과 375×812에서 홈 footer→개인정보처리방침→이용약관→개인정보처리방침 동선, HTTP 200, 비로그인 접근, 가로 넘침 없음, 마지막 링크 가림 없음과 console/page 오류 0건을 확인했다.
- agent-browser로도 두 문서의 내용, Next.js 오류 오버레이 없음, 모바일 `scrollWidth=clientWidth=375`, 상호 이동을 확인하고 전체 페이지 스크린샷을 육안 검토했다.
...
### 법적 고지 Task 4.3 — 사용자 승인 카카오 재심사 접수

- **Status**: done — 심사 결과 대기
- 2026년 8월 19일 사용자의 별도 명시적 승인 뒤 대상 채널 `그린러브`의 카카오 비즈니스 재심사를 접수했다.
- 매장 없음, 대표자 본인, 기존 전자증명서 인증 완료 상태를 사용했다. 사업자등록증 업체명 `디어 오키드`와 채널명 `그린러브`의 차이는 연관성 소명 단계에서 설명했다.
- 연관성 설명은 `사업자 ‘디어 오키드’가 카카오톡 채널 ‘그린러브’와 화훼 쇼핑몰 ‘그린러브’를 직접 운영합니다. 홈페이지 첫 화면의 운영 관계·공식 채널 링크·대표상품·하단 사업자정보와 개인정보처리방침·이용약관에서 동일 운영 주체를 확인할 수 있습니다.`로 제출했다.
- 홈페이지 주소는 `https://greenlove.co.kr`로 입력했다. 인허가 안내를 확인한 뒤 비대상으로 선택했고 사진·파일 첨부는 0건이다. 사업자등록증 원본, 비공개 계정 정보, 주민등록번호, 발급번호는 추가 제출하지 않았다.
- 최종 권리·정보 정확성 확인에 동의했고 성공 화면에서 요청 내용 `비즈니스 재심사 신청`, 요청일 `2026. 8. 19.`, 예상 기간 `영업일 기준 평균 3~7일`을 확인했다.
- 이번 외부 변경은 카카오 재심사 접수 1건뿐이다. 카카오 채널 정보·소식, 이메일·DNS·ALIGO·Firebase·운영 DB와 배포 설정은 변경하지 않았다.
- **Verify**: 카카오 재심사 성공 화면의 요청 내용·요청일·예상 심사 기간 확인, `git diff --check -- docs/plans/REPORT_kakao_business_channel_reapproval.md`

## 재개 지점

consumer 법적 고지를 main과 production에 반영하고 운영 검증과 카카오 비즈니스 재심사 접수를 완료했다. 현재 카카오 심사 결과를 기다린다. ALIGO와 회차 직배송 출시는 카카오 승인·반려 결과 확인과 최신 main 통합 전까지 계속 보류한다.
