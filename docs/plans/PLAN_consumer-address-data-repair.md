...
1. `git status --short`와 `git log -3 --oneline`으로 현재 문서 커밋 범위를 확인한다.
2. `docs/specs/ops/seller-validation-data-cleanup.md`와 이 문서를 읽는다.
3. 운영 홈과 `/stores`를 읽기 전용으로 확인해 깨진 주소가 보이는 상점명과 가능한 `storeId`를 기록한다.
4. 백업·dry-run 준비 전에는 운영 쓰기를 하지 않는다.
