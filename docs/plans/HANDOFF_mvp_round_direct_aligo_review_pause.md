...
## 한 줄 상태 — 2026-08-22 KST

consumer 개인정보처리방침·이용약관의 main·production 반영, 카카오 비즈니스 채널 승인, ALIGO 발신 프로필 1건 등록과 `senderkey` 발급은 완료됐다. Task 12에서 내부 논리 코드↔ALIGO `tpl_code` 분리와 누락 본문 변수 3종을 로컬 코드·테스트·정본에 구현했다. 회차 알림 템플릿 8종은 아직 등록·승인하지 않았고 운영 배포·첫 회차 생성·판매 모드 전환도 실행하지 않았으므로 회차 직배송 출시는 계속 중단한다.
...
| ALIGO 발신 프로필 | 정상 등록 1건 |
| `senderkey` | 발급 완료, 원문 미기록 |
| 로컬 알림 계약 구현 | 내부↔외부 코드 분리와 누락 본문 변수 3종 구현·테스트 완료 |
| 회차 알림 템플릿 8종 | 미등록·미승인 |
| 실제 알림톡·SMS fallback 검증 | 미실행 |
| 운영 ALIGO 자격 증명 4개·템플릿 코드 매핑 | 미등록 |
| 회차 출시 후보 운영 배포 | 미실행 |
| 첫 회차 | 미생성 |
| `salesMode` | `legacy` 유지 |
| PR #11 | `OPEN`·초안·`MERGEABLE`, `mergeStateStatus: CLEAN`, 자동 검사 5개 성공 |

카카오 승인, ALIGO 발신 프로필·`senderkey` 준비, 최신 main 통합·원격 동기화·PR #11 재검증, 동일 원격 head E2E 52건, Task 12 로컬 구현까지 완료됐다. 그러나 회차 템플릿 8종의 provider 등록·승인과 후속 실제 발송은 완료되지 않았다. 다음 외부 작업은 별도 승인 범위에서만 수행한다.
...
- PR #11을 병합하지 않는다.
- 추가 ALIGO 발신 프로필·템플릿 등록·실제 발송 작업은 해당 외부 게이트 승인 없이 진행하지 않는다.
...
5. ALIGO 발신 프로필 등록과 `senderkey` 발급 여부 확인을 완료했다.
6. 내부 논리 템플릿 코드와 ALIGO `tpl_code`의 매핑 계층, `ORDER_ACCEPTED.name`, `ORDER_DELIVERY_HELD.reason`, `ORDER_CANCELLED.reason` 차단점의 로컬 구현을 완료했다.
7. 실제 도달 가능한 회차 알림 템플릿 8종을 등록하고 모두 승인 상태인지 확인한다.
8. 승인된 격리 수신자에게 실제 알림톡 정상 발송과 SMS fallback을 검증한다.
9. 운영 ALIGO 자격 증명 4개와 `ALIGO_TEMPLATE_CODES_JSON`을 값 비공개 방식으로 반영하고 존재·매핑 검사를 통과시킨다.
...
- [x] 내부↔외부 템플릿 코드 분리와 누락 본문 변수 3종 로컬 구현
- [ ] 회차 알림 템플릿 8종 승인
