...
- 최종 기능 HEAD: `bc51e4dd729b6c5d731a6f79baab0ea91418422a`
- 최신 main 병합 SHA: `c96db6f3dd0ed5c7f6bd97fca9ab5763b9aed74c`
- 기능 PR: #20·#24, 모두 `MERGED`
...
- `apps/consumer/src/app/products/[id]/_components/ProductActions.tsx`
  ...
  - 스토어명, 로고와 로고 실패 시 첫 글자 아바타는 기존 동작을 유지한다.
  - 스토어 데이터의 `store.ceoName`이 `난플렉스`로 표시되던 보조 줄은 상품 상세에서 제거했다.
- `apps/consumer/src/app/products/[id]/_components/ProductActions.test.mjs`
  ...
  - `store.ceoName`이 판매자 정보에 다시 렌더링되지 않는 계약을 추가했다.
...
- 운영 모바일 375×812:
  ...
  - 가로 넘침과 오류 화면 없음.
- 최신 운영 데스크톱·375×812:
  - 상호·주소·고객센터는 유지되고 `난플렉스`는 0건이다.
  - 가로 넘침과 브라우저 콘솔 오류는 0건이다.
...
- 후속 기능 commit: `bc51e4d` — `consumer 상품 판매자 보조 문구를 제거한다`
- 원격 작업 branch에 push했다.
- PR #20의 검사가 모두 통과한 뒤 merge commit 방식으로 main에 병합했다.
- PR #24의 검사가 모두 통과한 뒤 merge commit `c96db6f3dd0ed5c7f6bd97fca9ab5763b9aed74c`로 main에 병합했다.
- consumer production:
  - 최신 Vercel `dpl_4TPxLkDExMDqxBUVRg8nSV6R86kv`, `READY`
  - GitHub deployment `5959074365`가 최신 main 병합 SHA와 일치하고 `greenlove.co.kr` 별칭 연결을 확인했다.
...
- 2026년 8월 18일 사용자 요청으로 카카오 비즈니스 4차 재심사를 접수했다. 채널 정보·이메일·DNS·ALIGO·공개 소식은 변경하지 않았다.
...
- 카카오 비즈니스 4차 재심사 결과는 대기 중이며 승인 자체는 보장되지 않는다.
...
3. 공개 상품 상세의 판매자 상호·주소·전화번호가 `PUBLIC_BUSINESS_INFO`와 일치하고 `난플렉스`가 표시되지 않는지 확인한다.
4. 카카오 심사 결과를 확인해 승인 또는 추가 보완으로 보고서를 갱신한다.
5. 사업자 연락처 변경 요청이 오면 운영 DB 대신 공개 사업자 정보 정본부터 갱신한다.
