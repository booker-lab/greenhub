...
- **결정**: `global-setup.ts`는 전체 E2E 시드를 다시 만들지 않고 `consumer@test.com` 소유의 송장 표시 주문 하나만 테스트 시작 전에 멱등 복구한다.
- **이유**: 전체 시드는 다른 테스트의 쓰기 상태를 덮어쓸 수 있고, fixture가 우연히 남아 있기를 기대하면 MY 상세 회귀가 404로 흔들린다.
- **계약**: 고정 주문은 `DELIVERED`·택배·`CJ대한통운`·운송장 `1234567890` 계약을 유지한다. MY E2E는 목록의 `확정 가능` 신호, 상세 송장, 구매 확정 실패·성공을 이 주문으로 검증한다.
