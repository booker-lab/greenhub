...
## [결정 #CL-145] 소비자 앱 육안검증 후속 작업은 탭별 단일 문서로 모은다 (2026-06-17)

- **결정**: 운영 도메인 육안검증 중 발견한 소비자 앱 후속 작업은 `docs/specs/frontend/consumer-app-visual-followup-plan.md`에 탭별로 모은다.
- **이유**: 홈, 카테고리, 상점, 공구, 장바구니, MY 탭을 따로 검증하면서 발견 사항이 분산되면 우선순위와 중복 여부를 판단하기 어렵다.
- **계약**: 기술적 버그 수정 여부와 UX 추가 개선 여부를 분리해 기록한다. 운영 데이터 정리 문제는 프론트 표시 개선과 구분하고, 구현이 필요한 항목은 별도 선 설계 또는 기존 탭별 문서로 승격한다.

## [결정 #CL-146] 소비자 상점 정렬은 드롭다운 대신 고정 버튼 컨트롤을 사용한다 (2026-06-17)

- **결정**: `/stores`의 정렬 선택은 Mantine `Select`가 아니라 `radiogroup` 역할의 고정 버튼 3개로 제공한다.
- **이유**: 운영 도메인 육안검증에서 `Select` 팝오버가 모바일·분할 화면 조건에서 큰 검은 도형과 화면 떨림을 만들었다. 정렬 옵션은 3개로 고정되어 있으므로 오버레이를 열 필요가 없다.
- **계약**: 정렬 옵션은 `가나다순`, `상품 수순`, `거점 수순`을 유지한다. E2E는 `radio` 역할로 정렬 선택을 검증하고, 화면 레이아웃을 흔드는 포털·팝오버 기반 UI를 사용하지 않는다.

## [결정 #CL-147] 소비자 앱 육안검증 후속 작업은 묶음별 배포·검증 게이트로 순차 실행한다 (2026-06-22)
...
## [결정 #CL-149] MY 인증 E2E는 전용 주문 fixture만 멱등 복구한다 (2026-06-22)

- **결정**: `global-setup.ts`는 전체 E2E 시드를 다시 만들지 않고 `consumer@test.com` 소유의 택배 완료·픽업 완료·공동구매 주문 3건만 테스트 시작 전에 멱등 복구한다.
- **이유**: 전체 시드는 다른 테스트의 쓰기 상태를 덮어쓸 수 있고, fixture가 우연히 남아 있기를 기대하면 MY 상세 회귀가 404로 흔들린다.
- **계약**: 택배 주문은 `DELIVERED`·`CJ대한통운`·운송장 `1234567890`, 픽업 주문은 `PICKED_UP`·픽업 코드 `482731`, 공구 주문은 `saleType=group` 계약을 유지한다. MY E2E는 일반/공구 구분, 상세 송장·픽업 코드, 구매 확정 실패·성공을 검증한다.

## [결정 #CL-150] 공구 탐색 화면의 참여 가능 노출은 `recruiting`으로 통일한다 (2026-06-22)

- **결정**: 홈 진행 중 미리보기와 카테고리 공동구매 탭은 공유 `getGroupBuyStatus()`가 `recruiting`을 반환한 상품만 노출한다. 공구 탭은 `recruiting`을 모집 중 영역에 두고 종료·실패·설정 오류를 각각 분리해 정보로 제공한다.
- **이유**: 홈은 설정 누락을 `모집 중`으로 대체하고 카테고리는 `saleType=group` 전체를 노출하는 반면, 공구 탭은 공유 상태를 엄격히 적용해 같은 상품이 화면마다 참여 가능한 것처럼 다르게 보였다.
- **계약**: `target_reached`, `deadline_closed`, `failed_minimum`, `missing_config`, `invalid_config`는 홈과 카테고리에서 숨긴다. `missing_config`와 `invalid_config`를 `모집 중`으로 표시하던 결정 #CL-142의 fallback은 이 결정으로 대체한다. 종료·오류 상태의 확인 경로는 공구 탭이 맡는다.

## [결정 #CL-151] 홈 공동구매 미리보기는 3열 컴팩트 탐색 카드로 유지한다 (2026-06-23)

- **결정**: 홈 공동구매 미리보기는 최대 3개를 같은 행에 두고, 이미지는 `1/1` 정사각형, 상품명과 수량은 각각 1줄로 제한한다.
- **이유**: W3 이후 홈에는 참여 가능한 `recruiting` 상품만 남으므로, 홈의 역할은 상세 설명보다 빠른 탐색이다. 기존 `4/5` 이미지와 2줄 상품명은 375px 첫 화면에서 다음 탐색 섹션을 밀어내 모바일 밀도를 낮췄다.
- **계약**: 마감 라벨은 이미지 내부 배지로 유지하고, 수량은 `현재/목표개 · N개 필요` 한 줄로 표시한다. 참여 가능 판정은 계속 공유 `getGroupBuyStatus()`의 `recruiting`을 사용하며, 상세 판단은 상품 상세와 공구 탭으로 넘긴다.

## [결정 #CL-152] 문서작업 세션 종료 시 다음 작업 핸드오프 프롬프트를 작성한다 (2026-06-23)

- **결정**: 소비자 앱 후속 실행 계획 같은 문서작업 세션이 종료되면, 마지막 응답 또는 세션 메모리에 다음 작업자가 바로 사용할 수 있는 핸드오프 프롬프트를 작성한다.
- **이유**: 묶음별 종결 기록만으로는 다음 세션의 첫 행동, 제외 범위, 검증 기준을 놓칠 수 있다. 명시적 핸드오프 프롬프트가 있으면 W0~Wn 재수행을 막고 다음 묶음을 바로 시작할 수 있다.
- **계약**: 핸드오프 프롬프트에는 완료 묶음, 재수행 금지 범위, 다음 묶음, 첫 행동, 필수 참조 문서, 검증·배포 게이트, 알려진 제약을 포함한다. `docs/memory.md`는 200라인 이하를 유지하며 최신 핸드오프 위치를 가리킨다.

## [결정 #CL-153] 카테고리 활성 필터 맥락은 URL 쿼리에서 파생한다 (2026-06-23)

- **결정**: 카테고리 화면의 활성 필터 라벨, 개별 해제, 전체 초기화, 빈 상태 복구 행동은 `parseCategoryQuery()`가 만든 URL 기반 상태에서 순수 유틸로 파생한다.
- **이유**: W5 범위는 현재 조건을 즉시 이해하고 안전하게 해제하는 맥락 개선이다. 별도 클라이언트 상태를 두면 새로고침·공유 링크·뒤로가기에서 조건 라벨과 실제 API 쿼리가 어긋날 수 있다.
- **계약**: `latest` 정렬은 기본값이라 라벨에서 제외하고, 공동구매·카테고리·색상·정렬 순서로 라벨을 만든다. 개별 해제는 기존 쿼리 계약의 해당 키만 제거하며, 전체 초기화는 `category`, `saleType`, `colors`, `sort`를 모두 제거해 `/category`로 복귀한다. 해제 컨트롤은 실제 `href` 링크로 렌더링해 hydration 전후와 운영 alias 모두에서 URL 해제가 동일하게 동작하게 한다.

## [결정 #CL-154] 상품 0개 상점은 준비 중으로 표시하고 구매 가능한 상점 뒤에 둔다 (2026-06-23)

- **결정**: `/stores` 목록은 상품 수가 1개 이상인 상점을 먼저 보여주고, 상품 0개 상점은 모든 정렬에서 하단에 둔다. 상품 0개 상점은 `준비 중` 배지와 `상품 준비 중` 문구를 표시한다.
- **이유**: 소비자 상점 탐색의 목적은 구매 가능한 판매자를 빠르게 찾는 것이다. 상품이 없는 상점을 같은 우선순위로 두면 구매 흐름이 없는 카드가 상단을 차지하고, 단순히 `상품 0개`만 보이면 운영 중인지 준비 중인지 오해할 수 있다.
- **계약**: 기본 상품 수 정렬 라벨은 `구매 가능순`으로 둔다. 검색 결과 안에서도 0개 상점은 숨기지 않고 하단 정렬만 적용한다. API 응답 필드 추가, 백엔드 count 계약 변경, `ProductCard` variant 분리는 W6 범위에서 하지 않는다.

## [결정 #CL-155] 소비자 상품 카드는 화면별 variant로 표시 책임을 분리한다 (2026-06-23)
...
## [결정 #CL-159] 소비자 공개 화면의 깨진 주소는 운영 원본 데이터 보정으로만 처리한다 (2026-06-23)

- **결정**: 홈 상점 미리보기와 `/stores`에서 보이는 깨진 주소 문자열은 `stores/{storeId}.address` 운영 데이터 정리 대상으로 분리하고, 소비자 프론트에서는 mojibake 추정 치환이나 임의 정상값 fallback을 추가하지 않는다.
- **이유**: 깨진 문자열을 화면에서 숨기면 운영 원본 훼손이 남아도 발견하기 어려워지고, 추정 복원은 실제 주소와 다른 값을 소비자에게 노출할 수 있다. 주소는 구매 신뢰와 배송 판단에 직접 연결되므로 원본 데이터의 백업·dry-run·승인 기반 보정으로 다뤄야 한다.
- **계약**: 주소 보정은 대상 `storeId`, 현재 원본 값, 정상 주소 후보, 백업 경로, 명시적 승인이 모두 기록된 뒤 `address` 최소 필드 쓰기로만 수행한다. 기존 `scripts/ops/repair-mojibake-data.mjs`의 상호명·이름 allowlist와 섞지 않고 주소 전용 옵션 또는 별도 스크립트로 확장한다. CSS preload warning은 hydration 오류, 콘솔 error, 화면 깨짐이 없으면 기능 결함과 분리된 낮은 우선순위 운영 진단 경고로 추적한다.

## [결정 #CL-160] 소비자 CSS preload warning은 기능 영향 증거가 없으면 문서 추적으로 종결한다 (2026-06-24)

- **결정**: 운영·Preview 소비자 화면에서 반복되는 CSS preload warning은 콘솔 error, page error, hydration 오류, 스타일 누락, 화면 깜빡임, 레이아웃 깨짐이 모두 0이면 구현 수정 없이 낮은 우선순위 운영 진단 항목으로 유지한다.
- **이유**: 수집된 warning은 `/stores`와 대표 상점 상세에서 동일 CSS asset이 load 직후 몇 초 안에 사용되지 않았다는 브라우저 성능 진단이며, 사용자 흐름이나 렌더링 실패를 만들지 않는다. 기능 영향 없는 경고를 즉시 코드 변경으로 연결하면 Next.js asset 최적화 경로와 화면 비즈니스 로직이 불필요하게 섞인다.
- **계약**: 재발 추적은 `docs/plans/PLAN_consumer-css-preload-warning.md`의 W0~W3 기준을 따른다. 동일 warning만 있고 판정 기준을 통과하면 배포 차단 조건으로 올리지 않는다. 콘솔 error, hydration 오류, 스타일 누락, 레이아웃 깨짐 중 하나라도 관찰되면 별도 선 설계 문서로 최소 수정 범위를 다시 확정한다.

## [결정 #CL-161] 소비자 후속 검증 캡처는 필요 시 증거 보강으로만 추가한다 (2026-06-24)

- **결정**: 소비자앱 후속 W0~W11 종료 판정은 기존 Preview·Production 배포 증거, 375px·1440px 육안검증 관찰 기록, CSS warning 추적 결과를 기준으로 유지한다. 별도 이미지 캡처 파일은 필수 잔여 작업으로 만들지 않는다.
- **이유**: 현재 남은 구현 작업이 없고, 문서화된 관찰 기록은 화면별 가로 넘침, 콘솔 error, hydration 오류, 핵심 문구 표시 여부를 이미 포함한다. 기능 결함 없이 이미지 파일만 추가하면 검증 기준보다 증거 형식이 작업을 끌고 가게 된다.
- **계약**: 새 결함, 배포 회귀, 외부 감사 요청처럼 재현 증거가 필요한 경우에만 `docs/plans/` 또는 관련 `docs/specs/`에 캡처 범위와 종료 기준을 먼저 고정한 뒤 캡처를 추가한다. 기능 수정, 성능 warning 추적, 캡처 보강은 서로 다른 작업으로 분리한다.
