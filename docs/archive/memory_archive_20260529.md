...
최종 수정: 2026-05-29 (어드민 users S4 백엔드 조회 상한·문서 정리 #CL-55)

---

## 진행 현황

**2026-05-29 작업 (어드민 users S1 D1 구현 진행 #CL-55)**:
- `admin-tab-users-plan.md` S1에 따라 `AuthService.refresh()`가 refresh token rotation 검증 후 `users/{userId}.suspended === true`이면 401을 반환하도록 보강했다. `apps/api/src/auth/auth.service.spec.ts`를 신설해 정지 사용자 refresh 차단과 정상 사용자 rotation 후 재발급 회귀를 함께 검증했다. API 명세 `docs/specs/api/auth.md`·`docs/specs/api/admin.md`도 refresh 차단 정책으로 갱신했다. **정합성**: api `tsc --noEmit` 0, auth spec 2/2, api build 0, 신규 spec 단독 ESLint 0, 500라인/Memory 한도 통과. **잔여**: `pnpm --filter api lint` 전체는 기존 ESLint 부채 409 errors/24 warnings로 실패, 수동 refresh API 401 확인은 미수행.

**2026-05-29 작업 (어드민 users S2 T1+T2 구현 #CL-55)**:
- `apps/seller/src/app/admin/users/_components/UsersTable.tsx`에 가입일·전화 표시를 추가하고, 가입일은 shared `toDateStrKST`로 KST 보정했다. `apps/seller/src/app/admin/users/_client.tsx`에는 `useAdminUsers().reload` 기반 새로고침 아이콘 버튼을 추가했다. 브라우저 확인 중 비로그인 세션의 `session.user` 누락으로 `proxy.ts`가 터지는 문제를 발견해 `/login` 리다이렉트 가드를 `!session?.user`로 보강했다. **정합성**: seller `tsc --noEmit` 0, seller build 0, 신규 변경 lint 경고 0, 라인 한도 통과. **잔여**: seller lint 전체는 기존 `<img>` 경고 2건 유지, 로컬 Firebase/Auth env가 invalid-api-key라 admin 세션 육안 확인은 미완료.

**2026-05-29 작업 (어드민 users S3 T3 검색·상태 필터 구현 #CL-55)**:
- `apps/seller/src/app/admin/users/_lib.ts`에 `filterUsers`·`getUserEmptyKind`·상태 탭 SSOT를 신설하고, `UsersFilters.tsx`를 추가해 이름·이메일·전화 검색과 전체/정상/정지 필터를 `_client.tsx`에 연결했다. 전화 검색은 하이픈 제거 후 매칭하고, 검색어는 Mantine `useDebouncedValue(..., 200)`로 적용한다. **정합성**: seller tsc 0, `_lib.test.ts` 8/8, seller build 0, seller lint 종료 코드 0(기존 `<img>` 경고 2건만), 신규 파일 500라인 한도 통과. **잔여**: 로컬 `/admin/users`는 인증 리다이렉트와 Firebase invalid-api-key/Auth 설정 오류로 관리자 화면 육안 확인 미완료.

**2026-05-29 작업 (어드민 users S4 T4 백엔드 limit·정렬 구현 #CL-55)**:
- `AdminService.getUsers()`에 `createdAt desc` 최신순과 `limit(5000)`을 적용하고, `firestore.indexes.json`에 `users(role ASC, createdAt DESC)` 복합 인덱스를 추가했다. `docs/specs/api/admin.md`, `CRITICAL_LOGIC.md`, `admin-tab-users-plan.md`, `pending-visual-verify.md`를 갱신해 무제한 읽기 방지·인덱스 배포·최신순 육안 확인을 추적한다. **정합성**: api tsc 0, api build 0, auth spec 2/2, seller users vitest 8/8, seller tsc 0, seller build 0, seller lint 종료 코드 0(기존 `<img>` 경고 2건만). **잔여**: api ESLint는 기존 `any` 계열 부채로 실패, Firestore 인덱스 운영 배포와 `/admin/users` 최신순·5000건 상한 영향은 육안/운영 확인 필요.
