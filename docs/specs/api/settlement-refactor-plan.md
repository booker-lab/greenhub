...
| ~~**S5**~~ ✅ | F-2 (F-3 미진행) | 🟢 어드민 구조 분리 + N10(정산일시·정렬 desc 통일)·N11(합계 confirmed+paid 한정) — 세션79 완료(셀러 tsc+API build) | F-1(공유 타입) |
| **S6** | T-검증 + T-기록 | 통합 검증(e2e·육안) + #CL-44/45 적용 결과 기록·메모리. **← 차기(구현 전부 종결, 검증만 잔여)** | 전 세션 |
