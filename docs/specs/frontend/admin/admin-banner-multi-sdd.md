...
## 8. 2026-05-30 S5 진행 현황

- [x] **S5-U7 hook 재구성** — `useAdminBanners`를 별도 훅으로 분리하고 `/admin/banners` 목록 조회, 생성, 수정, 삭제를 한 진입점에서 처리하도록 바꿨다. 기존 `useAdmin.ts`는 500라인 한도 보호를 위해 배너 훅을 재수출만 한다.
- [x] **S5-U4 목록 화면** — `/admin/banner`를 기본/기간 배너 목록으로 재구성했다. 기본 배너는 삭제 버튼을 숨기고, 기간 배너는 노출 중·예정·만료 배지와 기간을 표시한다.
- [x] **S5-U5 추가/수정 Drawer** — 기존 이미지·텍스트·CTA 섹션을 재사용하고 기간 배너 전용 시작일·종료일 입력을 추가했다. 기본 배너 수정 시 기간 입력은 숨긴다.
