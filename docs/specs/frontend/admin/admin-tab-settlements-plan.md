...
- **A1 입금 시각 표시 완료(2026-06-03, #CL-81)**: 별도 SDD `admin-settlements-payment-timing-plan.md`를 먼저 확정하고, 데스크톱 행·모바일 카드의 상태 Badge 아래에 `paidAt` 기반 `입금 완료 YYYY-MM-DD HH:mm`, `confirmedAt` 기반 `지급 대기 · 확정 YYYY-MM-DD HH:mm`을 추가했다. 백엔드에 없는 입금 예정일은 추정하지 않는다. 로컬 fixture 인증 회귀 chromium·mobile 24/24, 포맷 함수 단위 테스트 4/4, seller 타입체크, 변경 파일 Biome을 통과했다. `pending-visual-verify.md` §6 `#85` 종결.
- **F4 스토어명 표시·Select화 진행(2026-06-03, #CL-82)**: 별도 SDD `admin-settlements-store-name-plan.md`를 확정했다. 기존 `useAdminStores()`로 표시 사전을 만들고, 정산 API의 `storeId` 계약은 유지한다. 데스크톱 표·모바일 카드·체크박스 접근성 이름은 스토어명 우선, 누락 시 축약 ID 대체 표시를 사용한다.
- **F4 스토어명 표시·Select화 완료(2026-06-03, #CL-82)**: 정산 필터를 검색 가능한 Mantine `Select`로 교체하고, 데스크톱 표·모바일 카드·체크박스 접근성 이름에 같은 스토어명 사전을 연결했다. 목록에 없는 ID는 축약 ID로 대체한다. 로컬 seller 인증 환경에서 `admin-settlements.spec.ts` chromium·mobile 24/24, seller 타입체크, seller 빌드, 변경 파일 Biome을 통과했다.
