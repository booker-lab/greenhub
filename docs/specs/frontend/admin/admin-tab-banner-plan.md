...
### 그룹 A — 결함 해소 (저위험·선결)
- **T1 (G-2). useEffect 의존성 교정** — deps `[banner]`로 좁힘, hydrate 의도 명확화
  (입력 유실 방지). `eslint-disable` 제거 가능 여부 확인. (독립, 최우선)
- **T2 (G-3). CTA 비대칭 입력 검증** — label↔href 둘 중 하나만 채운 경우 어드민 저장 차단·안내.
  (독립)
- **T3 (G-4). 에러 표시** — `save`/`handleImageUpload` 실패를 토스트·인라인 메시지로 노출.
...
- **T6 (R1). CTA 반응형** — 모바일에서 2열→1열 또는 `wrap`. 시각 미세 변경.
