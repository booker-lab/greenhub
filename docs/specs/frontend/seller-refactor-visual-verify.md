...
> 작성: 2026-05-19 (세션45 종료 후) · 최종 갱신: 2026-05-23 (세션82 — E 정산 섹션 + M-PATH 마스터 통합 동선 신설)
> 목적: 완료된 셀러앱·소비자앱 프론트 리팩토링 결과를 **브라우저에서 사람이 직접 눈으로** 확인.
> 범위: 코드/타입체크/e2e가 아닌 **실사용 화면 검증** 전용. 리팩토링 세션이 끝날 때마다 이 문서에 섹션을 추가한다.
>
> ## 🎯 다음 세션 진입점 — 문서 맨 끝 **[M-PATH](#m-path--셀러앱-리팩토링-전체af-마스터-통합-육안-검증-동선-세션82-신설)** 로 바로 가시오.
> 셀러앱 리팩토링 전 범위(A~F)를 **로그인 1회 동선(M0~M6)**으로 묶은 마스터 체크리스트가 문서 끝에 있습니다. 아래 A~F·V-PATH는 구간별 상세 통과 기준의 **참조 원본**(M-PATH 각 행이 연번으로 가리킴). 처음부터 끝까지 읽을 필요 없이 M-PATH만 따라가면 됩니다. 시드 안내는 M0·각 M구간 머리말 참조.
...
| # | 확인 항목 | 통과 기준 | 결과 | 원본 |
|---|----------|----------|:----:|------|
| 226 | PageHeader 중앙 홈 아이콘 | 홈 아닌 페이지 헤더 정중앙에 🏠, 홈에선 숨김 | [ ] | A-T1 #1~4 |
| 227 | 오늘 할 일 카드 | 최상단, 0건 줄 숨김, 모두 0이면 "마쳤어요 🎉" | [ ] | A-T3 #7~11 |
| 228 | 주문·정산·상품 현황 카드 | 파이프라인 4칸·정산 예정 금액·상품 판매중/비활성 + "더보기 >" | [ ] | A-T4 #12~16 |
| 229 | BottomNav 구성 | `주문·상품·정산·준비·설정`(거점 탭 없음, 설정→거점관리) | [ ] | A-T5 #17~20 |
| 230 | 준비 물량 탭(`/prep`) | 상품별 집계표 + "N개 상품·총 N개" + 지연 섹션 | [ ] | A-T6 #21~25 |
...
| # | 확인 항목 | 통과 기준 | 결과 | 원본 |
|---|----------|----------|:----:|------|
| 231 | 상태 탭 시각(F-UX1) | active=초록·700, inactive 회색. sticky=헤더 바로 아래. count Badge(0건 숨김) | [ ] | V1 #159~164 |
| 232 | 상태 뱃지 색 통일(B-T1) | 접수=주황·준비중=파랑·배송중=보라·완료=초록·취소=빨강, 카드↔상세 일치 | [ ] | B-T1 #28~31 |
| 233 | 요약바 제거·OrderCard 경량화(B-T2·B-T3) | 가로 요약바 없음. 카드 "준비 시작" 탭→상세 이동(인라인 폼·강제취소 없음) | [ ] | B-T2 #32~35·B-T3 #36~40 |
| 234 | 주문 상세 sticky footer(B-T4) | 액션 버튼 하단 고정, BottomNav와 안 겹침, 읽기전용은 footer 미노출 | [ ] | B-T4 #41~45 |
| 235 | 날짜 필터 칩 + 그룹 헤더(B-T5·B-T6) | `[오늘][이번주][이번달][직접입력]` 기본 이번주. 날짜 섹션 헤더·지연 최상단 빨강 | [ ] | B-T5 #46~50·B-T6 #51~57 |
| 236 | 일반/공구 토글(D-T4) | 헤더 아래 `일반 주문·공동구매` 토글, 일반=날짜칩 노출·공구=칩 미노출 | [ ] | D-T4 #74~82 |
| 237 | 공구 배송일 조인(D-T5) | 공구 토글 시 `groupDeliveryDate` 기준 날짜 그룹, 미설정은 "날짜 미정" | [ ] | D-T5 #83~87 |
...
| # | 확인 항목 | 통과 기준 | 결과 | 원본 |
|---|----------|----------|:----:|------|
| 238 | 필터 탭·카운트(F-UX1) | 주문 탭과 동일 패턴, 라벨 `전체 N` 인라인, non-sticky | [ ] | V2 #165~167 |
| 239 | 상품 카드 Switch+Button(F-UX2) | 상품명 우측 Switch(초록), 액션 row 수정(gray)·삭제(red) Button. Badge 잔존 0 | [ ] | V2 #168~173 |
| 240 | 상품 삭제 ConfirmModal(F-UX3) | 삭제 클릭→모달에 상품명 동적 표시, 로딩 중 외부클릭 차단 | [ ] | V2 #174~175 |
| 241 | 상품 등록 이미지 라벨(F-UX4c) | `/products/new` 80×80 이미지칸 "대표"·순번·✕·"사진 추가" 라벨 잘림/줄바꿈 없음(폰트 12px) | [ ] | V3 #176~180 ⚠️위험 |
...
| # | 확인 항목 | 통과 기준 | 결과 | 원본 |
|---|----------|----------|:----:|------|
| 242 | 정산 탭 시각·sticky(F-UX1) | 주문 탭과 동일 패턴, sticky=`var(--header-height)`(매직넘버 57 없음) | [ ] | V4 #184~185 |
| 243 | 정산 목록 정렬 desc(E-T1) | 정산일시 기준 **최신순(desc)** — 위→아래 내림차순(S5 N10 회귀) | [ ] | E-T1 #211 |
| 244 | 상태 라벨·색(E-T1) | pending="정산 대기"/노랑 등 shared SSOT 라벨·색 | [ ] | E-T1 #212~213 |
| 245 | status 필터 동작(E-T1) | 상태 필터 변경 시 목록 즉시 갱신(백엔드 연결) | [ ] | E-T1 #214 |
| 246 | 어드민 정산일시 컬럼·desc·합계(E-T2) | (admin) 정산일시 컬럼·desc 정렬·라벨/색 셀러와 일치·합계 confirmed+paid 한정 | [ ] | E-T2 #215~218 |
| 247 | 어드민 "지급처리" 버튼 노출(E-T3) | (admin) **confirmed 행에만** "지급처리" 버튼(pending엔 미노출)→클릭 ConfirmModal(blue)→paid | [ ] | E-T3 #221 + V9 #202 |
...
| 249 | 일일 캡·배송 설정 폰트(F-UX4b) | `/settings/daily-caps` 셀 카운트 12px·입력 15px / `/settings/delivery` 비용 입력 15px | [ ] | V6·V7 #190~193 |
| 250 | 거점 삭제 ConfirmModal(F-UX3) | `/hubs` 삭제→모달 제목 "거점 삭제"·confirm "삭제"(red), 처리 중 외부클릭 차단 | [ ] | V8 #194~195 |
| 251 | 관리자 모달 색상 분기(F-UX3) | (admin) 드라이버 승인=초록·정지=빨강·해제=회색 / 사용자 정지=red·해제=green | [ ] | V9 #199·206 |

## M6 — 회귀 가드 종합 (화면 무관 / 콘솔)

| # | 확인 항목 | 통과 기준 | 결과 | 원본 |
|---|----------|----------|:----:|------|
| 252 | 콘솔 에러 0 | M1~M5 동선 진행 중 DevTools Console 빨강 에러 0건(정상 로그 제외) | [ ] | V10 #209 |
