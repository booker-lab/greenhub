...
| API·도메인 계약 | 직접 관련 `docs/specs/` 현행 명세 |
...
| 완료 PLAN·REPORT·과거 보안/프론트 계획 | 역사 증거가 필요할 때만 검색 |

동작 계약이 충돌하면 현재 `main`의 코드·설정·테스트를 기준으로 현행 spec을 고친다. 운영·작업 상태가 충돌하면 직접 재검증 결과 → `docs/memory.md` → 활성 HANDOFF·PLAN → 역사 자료 순으로 판정한다.
...
|---|---|---|---|
| consumer | 대상 `src/app/**`, 관련 hook/lib/api/auth | 회차 직배송 + 직접 관련 API spec | HTTP 계약이면 API, 공통 DTO면 shared, Firebase 직접 접근이면 Rules, 결제면 payments, 다역할이면 E2E |
| seller/admin | 대상 page, hook, lib/api, auth | admin·orders·products·hubs·settlements | mutation/응답이면 API, 공통 상태면 shared, Firebase 직접 접근이면 Rules |
| driver | board/map/profile, lib/api, auth | orders·auth·회차 직배송 | 상태 전이면 API, 사진이면 delivery-photo/Storage Rules, 다역할이면 E2E |
| API | module/controller/service/DTO/spec | `docs/specs/api/README.md`에서 현행 domain spec 선택 | 공개 계약이면 shared와 소비 앱, Firebase 쿼리면 Rules/index, 공급자면 ops |
...
| 결제 | `apps/api/src/payments`, consumer payment flow | `docs/specs/api/payments.md` |
| 알림 | `apps/api/src/notifications` | `docs/specs/api/notifications.md` |
...
| Firebase 권한 | `firestore.rules`, `storage.rules`, indexes | `docs/security/threat-model.md` + 직접 관련 ops/spec |
| CI·Preview | `.github/workflows`, `apps/e2e`, scripts | 현행 workflow·E2E 문서. 완료된 `*-plan.md`는 역사 자료로 취급 |

`driver`, `operations`, `retention`, `stores`, `varieties` 등 전용 API spec이 없는 영역은 대상 코드와 인접 현행 spec을 먼저 사용한다. 실제 계약 공백이 있을 때만 새 명세를 만든다.
...
- 루트 `pnpm lint`는 수정형 lint가 포함될 수 있으므로 read-only 감사 명령으로 간주하지 않는다.
...
1. 주 영역 하나로 시작한다.
2. 대상 package 설정, 파일, 직접 test, 현행 spec만 먼저 읽는다.
...
6. 여러 역할 Acceptance Criteria가 있을 때만 E2E 전체 흐름을 연다.
7. 운영 데이터·배포·외부 공급자 작업일 때만 scripts/infrastructure를 연다.
8. 완료 PLAN·REPORT·archive는 회귀 원인 또는 결정 충돌을 추적할 때만 읽는다.
9. 필요한 근거를 확보하면 확장을 중단한다.
